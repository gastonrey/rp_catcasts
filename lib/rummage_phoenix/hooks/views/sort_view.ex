  ...
  defmacro __using__(opts) do
    quote do
      def sort_link(conn, rummage, link_params) do
        ...
        {sort_field, current_order} =
          get_sort_field_and_current_order(current_sort_params, field, assoc)
        ...
        raw(
          link_with_icon(
            ...
            apply(unquote(opts[:helpers]), String.to_atom("#{unquote(opts[:struct])}_path"), [
              conn,
              :index,
              %{rummage: Map.put(rummage, "sort", sort_params)}
            ]),
            current_order,
            asc_sort_icon,
            asc_sort_text,
        ...
        case current_order do
          "asc" ->
            case asc_sort_icon do
              nil ->
                """
                <a class="text-grey-darkest font-semibold uppercase text-sm no-underline" href="#{
                  url
                }">#{name} #{asc_sort_text}</a>
                """
              ...
              _ ->
                ...
                <a class="text-grey-darkest font-semibold uppercase text-sm no-underline" href="#{
                  url
                }">#{name}  <img src=\"#{asc_sort_icon}\" alt=\"#{asc_sort_text}\" title=\"#{
                  asc_sort_text
          ...
          "desc" ->
            case desc_sort_icon do
              ...
              _ ->
                ...
                <a class="text-grey-darkest font-semibold uppercase text-sm no-underline" href="#{
                  url
                }">#{name}  <img src=\"#{desc_sort_icon}\" alt=\"#{desc_sort_text}\" title=\"#{
                  desc_sort_text
                }\" height=\"10\" width=\"10\"></a>
                """
            end
          ...
          _ ->
            """
            <a class="text-grey-darkest font-semibold uppercase text-sm no-underline" href="#{url}">#{
              name
            }</a>
            """
        end
      ...
      defp get_sort_field_and_current_order(current_sort_params, field, assoc) do
        cond do
          current_sort_params in [nil, "", [], %{}] ->
            {"#{field}.asc", nil}

          is_nil(current_sort_params["assoc"]) ->
            current_sort_field =
              current_sort_params["field"]
              |> String.split(".ci")
              |> Enum.at(0)
